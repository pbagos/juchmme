...
class Test
{
    ...
    private static void saveGraphPlot( Seq seq, double[][] total )
    {
        ...
        if( Params.VITERBI>-1 )
        { mypath[i] = seq.path[Params.VITERBI]; i++; }
        if( Params.POSVIT>-1 )
        { mypath[i] = seq.path[Params.POSVIT]; i++; }
        if( Params.PLP>-1 )
        { mypath[i] = seq.path[Params.PLP]; i++; }
        if( Params.NBEST>-1 )
        { mypath[i] = seq.path[Params.NBEST]; i++; }
        if( Params.DYNAMIC>-1 )
        { mypath[i] = seq.path[Params.DYNAMIC]; i++; }
