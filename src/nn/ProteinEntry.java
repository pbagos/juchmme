...
public class ProteinEntry {
	...
	public ArrayList<double []> getBinaryAAwindows(int windowSize) {   
		...
		String aminoAcidSeqFlanked = FlankAASeq(windowSize);
	 ...
	 * @param windowSize is an int corresponding to the desired sliding window size
	 */	
	private String FlankAASeq (int windowSize) 
	{
		String flankAA="";
		String aminoAcidSeqFlanked=this.aminoAcidSequence;
		for(int i=0;i<((windowSize-1)/2);i++){
			flankAA = flankAA.concat("#");
		}
		aminoAcidSeqFlanked = flankAA.concat(aminoAcidSeqFlanked);
		aminoAcidSeqFlanked = aminoAcidSeqFlanked.concat(flankAA);
